...
(async function () {
  ...
  while (1) {
    ...
    const payload = new TimeboxEvoMessage(message).message; // eslint-disable-next-line no-await-in-loop
    await bluetooth.writeMessage(payload); // eslint-disable-next-line no-await-in-loop
    await sleep(1000);
  }
})();
